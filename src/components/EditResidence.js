...
import './EditResidence.css'; // Import CSS for styling
...
const EditResidence = () => {
    ...
    useEffect(() => {
        const fetchResidence = async () => {
            ...
            if (token) {
                try {
                    const response = await axios.get(`https://bizapp-40fj.onrender.com/api/residences/${id}/`, {
                        headers: {
                    ...
                    setFormData({
                        ...
                        residence_type: response.data.residence_type,
                        room_price: response.data.room_price,
                        rooms_include: response.data.rooms_include,
                        description: response.data.description,
                        cover_image: response.data.cover_image,
                        rooms_available: response.data.rooms_available,
                        number_of_rooms_available: response.data.number_of_rooms_available,
                        room_available_date: response.data.room_available_date,
                        images: response.data.images,
                        videos: response.data.videos,
                        business_contacts: response.data.business_contacts,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`https://bizapp-40fj.onrender.com/api/residences/${id}/`, residenceData, {
                headers: {
                    'Authorization': `Token ${token}`,
            ...
            setMessage('An error occurred while updating the residence.');
            console.error(error);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="edit-residence-container">
            <h1 className="title">Edit Residence</h1>
            <form onSubmit={handleSubmit} className="edit-residence-form">
                ...
                <button type="submit" disabled={loading} className="btn btn-primary">
                    {loading ? 'Submitting...' : 'Update Residence'}
                </button>
            </form>
